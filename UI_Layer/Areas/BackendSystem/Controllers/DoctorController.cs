...
namespace UI_Layer.Areas.BackendSystem.Controllers
{
    ...
    public class DoctorController : Controller
    {
        ...
        public DoctorController(IWebHostEnvironment webHostEnvironment, IStringLocalizer<Resource> localizer, ApplicationDbContext dbContext, UserManager<ApplicationUser> userManager)
        {
        ...
        [HttpPost]
        public ActionResult DoctorInfoReport(CustomerFilterViewModel viewModel)
        {
            ...
            try
            {
                ...
                //var fromDate = Convert.ToDateTime(Convert.ToDateTime(viewModel.FilterFromValue).ToString("yyyy/MM/dd"));
                //var toDate = Convert.ToDateTime(Convert.ToDateTime(viewModel.FilterToValue).ToString("yyyy/MM/dd"));
                DateTime.TryParse(viewModel.FilterFromValue, out var fromDate);
                DateTime.TryParse(viewModel.FilterToValue, out var toDate);
                    ...
                    switch (viewModel.FilterKey)
                    {
                        case "CreatedDate":
                            _list = _dbContext.Customer
                                .Where(w => w.CreatedDate.Date >= fromDate.Date && w.CreatedDate.Date <= toDate.Date)
                        ...
                        case "Birth":
                            _list = _dbContext.Customer
                                .Where(w => w.DateOfBirth >= fromDate && w.DateOfBirth <= toDate)
                                .Include(customerDm => customerDm.ApplicationUser)
                                .ToList();
                            break;
                    }
        ...
        public ActionResult DoctorInfoFilterReport(string viewModel)
        {
