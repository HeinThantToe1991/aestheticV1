function getRandomColor() {
    var letters = '0123456789ABCDEF'.split('');
    var color = '#';
    for (var i = 0; i < 6; i++) {
        color += letters[Math.floor(Math.random() * 16)];
    }
    return color;
}

//#region Jquery DataTable Export Customize Function
function pdfFunction(doc, id) {
    var objLayout = {};
    objLayout['hLineWidth'] = function (i) { return .5; };
    objLayout['vLineWidth'] = function (i) { return .5; };
    objLayout['hLineColor'] = function (i) { return '#aaa'; };
    objLayout['vLineColor'] = function (i) { return '#aaa'; };
    objLayout['paddingLeft'] = function (i) { return 4; };
    objLayout['paddingRight'] = function (i) { return 4; };
    doc.content[1].layout = objLayout;
    var obj = {};
    obj['hLineWidth'] = function (i) { return .5; };
    obj['hLineColor'] = function (i) { return '#aaa'; };

    var visibleColCount = $('#' + id).DataTable().columns(':visible').count();

    var allVisible = $('#' + id).DataTable().columns().visible();
    var widtharray = [];
    var percent_ = 0;
    var minuspercent_ = 0;
    for (var i = 0; i < visibleColCount; i++) {

        var wpercent = (100 - percent_) / (visibleColCount - minuspercent_);
        widtharray.push(wpercent + "%");
    }
    doc.content[1].table.widths = widtharray;//Total Must Be "100"   
}

function pdfFunctionWithSrNo(doc, id) {
    var objLayout = {};
    objLayout['hLineWidth'] = function (i) { return .5; };
    objLayout['vLineWidth'] = function (i) { return .5; };
    objLayout['hLineColor'] = function (i) { return '#aaa'; };
    objLayout['vLineColor'] = function (i) { return '#aaa'; };
    objLayout['paddingLeft'] = function (i) { return 4; };
    objLayout['paddingRight'] = function (i) { return 4; };
    doc.content[1].layout = objLayout;
    var obj = {};
    obj['hLineWidth'] = function (i) { return .5; };
    obj['hLineColor'] = function (i) { return '#aaa'; };

    var visibleColCount = $('#' + id).DataTable().columns(':visible').count();
    var allVisible = $('#' + id).DataTable().columns().visible();
    var colSrno = $('#' + id).DataTable().columns().visible()[0];
    var widtharray = [];
    var percent_ = 0;
    var minuspercent_ = 0;
    if (colSrno) {
        widtharray.push(10 + "%");
        visibleColCount = visibleColCount - 1;
        for (var i = 0; i < visibleColCount; i++) {

            var wpercent = (90 - percent_) / (visibleColCount - minuspercent_);
            widtharray.push(wpercent + "%");
        }
    } else {
        for (var i = 0; i < visibleColCount; i++) {

            var wpercent = (100 - percent_) / (visibleColCount - minuspercent_);
            widtharray.push(wpercent + "%");
        }
    }

    doc.content[1].table.widths = widtharray;//Total Must Be "100"
}

function RemoveReadMoreInpdfFunction(doc, id) {
    var table = $('#' + id).DataTable();
    var tbody = doc.content[1].table.body;
    var objLayout = {};
    objLayout['hLineWidth'] = function (i) { return .5; };
    objLayout['vLineWidth'] = function (i) { return .5; };
    objLayout['hLineColor'] = function (i) { return '#aaa'; };
    objLayout['vLineColor'] = function (i) { return '#aaa'; };
    objLayout['paddingLeft'] = function (i) { return 4; };
    objLayout['paddingRight'] = function (i) { return 4; };
    doc.content[1].layout = objLayout;
    var visibleColCount = table.columns(':visible').count();
    var widtharray = [];
    var percent_ = 0;
    var minuspercent_ = 0;
    for (var i = 0; i < visibleColCount; i++) {
        var wpercent = (100 - percent_) / (visibleColCount - minuspercent_);
        widtharray.push(wpercent + "%");
    }
    for (var i = 1; i < tbody.length; i++) {
        for (var j = 0; j < tbody[i].length; j++) {
            tbody[i][j].text = tbody[i][j].text.replace("Read more", "");
        }
    }
    doc.content[1].table.body = tbody;
    doc.content[1].table.widths = widtharray;//Total Must Be "100"   
}

function RemoveReadMoreInExcelFunction(doc) {
    //_customizeExcelOptions(doc);
    var df = doc.xl.worksheets['sheet1.xml'].childNodes[0].childNodes[1].childNodes;
    for (var i = 1; i < doc.xl.worksheets['sheet1.xml'].childNodes[0].childNodes[1].childNodes.length; i++) {
        for (var j = 0; j < doc.xl.worksheets['sheet1.xml'].childNodes[0].childNodes[1].childNodes[i].childNodes.length; j++) {
            doc.xl.worksheets['sheet1.xml'].childNodes[0].childNodes[1].childNodes[i].childNodes[j].innerHTML = doc.xl.worksheets['sheet1.xml'].childNodes[0].childNodes[1].childNodes[i].childNodes[j].innerHTML.replace("Read more", "");
        }
    }
}

function RemoveReadMoreInCSVFunction(doc) {
    //doc = doc.replace("Read more", "");
    //return doc;
    //_customizeExcelOptions(doc);
    //console.log(doc);
    var data = '';
    //data = data.replace(/"/g, "'");
    //split at each new line
    splitData = doc.split('\n');
    //data = '';
    //for (i = 0; i < splitData.length; i++) {
    //    if (i == splitData.length -1) {
    //        data += splitData[i];
    //    } else {
    //        data += splitData[i] + '\n';
    //    }
    //}
    //splitData = data.split('\n');
    //data = '';
    for (i = 0; i < splitData.length; i++) {
        //add escaped double quotes around each line
        //data += splitData[i] + '\n';
        if (i == splitData.length - 1) {
            data += splitData[i].replace("Read more", "");
        } else {
            data += splitData[i].replace("Read more", "") + '\n';
        }
        //data += '\"' + splitData[i] + '\n"';
        //if its not the last line add CHAR(13)
        //if (i + 1 < splitData.length) {
        //    data += '\n ';
        //}
    }
    //Add concat function
    //data = 'CONCATENATE(' + data + ')';
    //doc = data;
    //doc = doc.replace("Read more", "");
    return data;
}

function RemoveReadMoreInPrintFunction(doc) {
    var data = doc.document.body.innerHTML;
    var splitData = data.split('<tr>');
    var res = '';
    for (i = 0; i < splitData.length; i++) {
        if (i == 0) {
            res += splitData[i];
        } else {
            res += '<tr>' + splitData[i].replace("Read more", "");
        }
    }
    doc.document.body.innerHTML = res;
    return doc;
}

function RemoveReadMoreInCopyFunction(doc) {
    var splitData = doc.split('\n');
    var res = '';
    for (i = 0; i < splitData.length; i++) {
        if (i == 0) {
            res += splitData[i];
        } else {
            res += '\n' + splitData[i].replace("Read more", "");
        }
    }
    return res;
}

//function pdfExportFunctionWithCompanyName(doc, headerName, name, address) {
//    //$(doc.document.body).find('h1')
//    //    .css('font-size', '20px')
//    //    .css('font-weight', 'bold')
//    //    .css('text-align', 'center')
//    //    .css('font-family', 'ZawgyiOne');


//    //doc.content.splice(0, 0, {
//    //    margin: [10, 10, 10, 12],
//    //    alignment: 'left',
//    //    /*  width: '10px',*/
//    //    width: 50,
//    //    height: 50,

//    //    image: 'data:image/png;base64,'
//    //    //image: '~/../../../dist/img/AdminLTELogo.png'
//    //});
//    doc.content.splice(0, 0, {
//        text: companyAddress,
//        alignment: 'center',
//        color: 'black',
//        fontSize: 12,
//        bold: true,
//        margin: [0, 0, 0, 8]
//    });
//    doc.content.splice(0, 0, {
//        text: companyName,
//        alignment: 'center',
//        color: 'black',
//        fontSize: 18,
//        bold: true,
//        margin: [0, 0, 0, 8]
//    });
//    doc.content[2] =
//        [{
//            alignment: 'left', text: headerName, fontSize: 18,
//            bold: true,
//        }, {
//            alignment: 'right', text: 'Print Date:' + formatDate(new Date()), fontSize: 12,
//            bold: true,
//        }]
//        //    [
//        //    {
//        //        alignment: 'left',
//        //        text: ['Test1']
//        //    },
//        //    {
//        //        alignment: 'right',
//        //        text: ['Test2']
//        //    },
//        //]
//        ;
//    //doc.content[2].alignment = 'left';
//    //doc.content[2].text = {
//    //    text: ["test"],
//    //};
//    //doc['header'] = (function (page, pages) {
//    //    return {
//    //        columns: [

//    //            {
//    //                // This is the right column
//    //                alignment: 'center',
//    //                text: [companyName]
//    //            },

//    //            //{
//    //            //    //if(true) {

//    //            //    //};
//    //            //    // This is the right column
//    //            //    alignment: 'right',
//    //            //    text: ['page ', { text: page.toString() }, ' of ', { text: pages.toString() }]
//    //            //}
//    //        ],
//    //        margin: [10, 10]
//    //    }
//    //});
//    doc['footer'] = (function (page, pages) {
//        return {
//            columns: [

//                {
//                    // This is the right column
//                    alignment: 'right',
//                    text: ['page ', { text: page.toString() }, ' of ', { text: pages.toString() }]
//                }
//            ],
//            margin: [40, 10]
//        }
//    });
//    console.log(doc);
//    //var objLayout = {};
//    //objLayout['hLineWidth'] = function (i) { return .5; };
//    //objLayout['vLineWidth'] = function (i) { return .5; };
//    //objLayout['hLineColor'] = function (i) { return '#aaa'; };
//    //objLayout['vLineColor'] = function (i) { return '#aaa'; };
//    //objLayout['paddingLeft'] = function (i) { return 4; };
//    //objLayout['paddingRight'] = function (i) { return 4; };
//    //doc.content[1].layout = objLayout;
//    //var obj = {};
//    //obj['hLineWidth'] = function (i) { return .5; };
//    //obj['hLineColor'] = function (i) { return '#aaa'; };

//    //var visibleColCount = $('#' + id).DataTable().columns(':visible').count();

//    //var allVisible = $('#' + id).DataTable().columns().visible();
//    //var widtharray = [];
//    //var percent_ = 0;
//    //var minuspercent_ = 0;
//    //for (var i = 0; i < visibleColCount; i++) {

//    //    var wpercent = (100 - percent_) / (visibleColCount - minuspercent_);
//    //    widtharray.push(wpercent + "%");
//    //}
//    //doc.content[3].table.widths = widtharray;//Total Must Be "100"   
//    //var colCount = new Array();
//    //$('table').find('tbody tr:first-child td').each(function () {
//    //    if ($(this).attr('colspan')) {
//    //        for (var i = 1; i <= $(this).attr('colspan'); $i++) {
//    //            colCount.push('*');
//    //        }
//    //    } else {
//    //        colCount.push('*');
//    //    }
//    //});
//    //doc.content[2].table.widths = colCount;

//}

function pdfExportFunctionWithCompanyName(doc, headerName, name, address) {   
    doc.content.splice(0, 0, {
        text: address,
        alignment: 'center',
        color: 'black',
        fontSize: 12,
        bold: true,
        margin: [0, 0, 0, 8]
    });
    doc.content.splice(0, 0, {
        text: name,
        alignment: 'center',
        color: 'black',
        fontSize: 18,
        bold: true,
        margin: [0, 0, 0, 8]
    });
    doc.content[2] =
        [{
            alignment: 'left', text: headerName, fontSize: 18,
            bold: true,
        }, {
            alignment: 'right', text: 'Print Date:' + formatDate(new Date()), fontSize: 12,
            bold: true,
        }];
    
    doc['footer'] = (function (page, pages) {
        return {
            columns: [
                {
                    alignment: 'right',
                    text: ['page ', { text: page.toString() }, ' of ', { text: pages.toString() }]
                }
            ],
            margin: [40, 10]
        }
    });
}

function printExportFunctionWithCompanyName(doc,removeName,companyName,companyAddress) {
    var data = doc.document.body.innerHTML.replace("<h1>"+removeName+" </h1>", "<h1>"+companyName+"</h1><h3>"+companyAddress+"</h3>");
    //var splitData = data.split('<tr>');
    var res = '';
    //for (i = 0; i < splitData.length; i++) {
    //    if (i == 0) {
    //        res += splitData[i];
    //    } else {
    //        res += '<tr>' + splitData[i].replace("Read more", "");
    //    }
    //}
    doc.document.body.innerHTML = data;
    //doc.document.body.innerHTML.replace("<h1>Stock</h1>", "<h1>Ace</h1><h1>address</h1>");
    return doc;
}
//#endregion Jquery DataTable Export Customize Function


/*var max_img_size = 102400;*/
function CheckSpecialCharacters(n) {
    var format = /[!@#$%^&*()_+\-=\[\]{};':"\\|, <>\/?]+/;
    if (format.test(n)) {
        return true;
    } else {
        return false;
    }
    //return returnName;
}

function CheckImageFileName(n) {
    if (n.length < 50) {
        return false;
    } else {
        return true;
    }
    //return returnName;
}

var maxImageSize = 102400;
function checkImageFileSize(f) {
    if (f.size > maxImageSize) {
        return true;
    }
    else {
        return false;
    }
}

function checkImage150KBSize(f) {
    var maxSize = 153600;
    if (f.size > maxSize) {
        return true;
    }
    else {
        return false;
    }
}

function clearImage(id) {
    $('#' + id).attr('src', 'data:image/jpeg;base64,' + "");
}

function fxAutoGenerateNoForImageName() {
    const date = new Date();
    const year = date.getFullYear();
    const month = String(date.getMonth() + 1).padStart(2, '0');
    const day = String(date.getDate()).padStart(2, '0');
    const hour = String(date.getHours()).padStart(2, '0');
    const minute = String(date.getMinutes()).padStart(2, '0');
    const secends = String(date.getSeconds()).padStart(2, '0');
    var res = year + month + day + hour + minute + secends;
    return res;
}

function GreaterThanZero(p) {
    if (p > 0) {
        return true;
    } else {
        return false;
    }
}

function quantityKeyDown(e) {
    var quantityvalue = QuantityTextBox_Keydown(e, e.target.id);
    if (quantityvalue === false) {
        return false;
    }
}

function quantityKeyUp(e) {
    var quantityvalue = QuantityTextBox_Keyup(e, e.target.id);
}

function isNullOrEmpty(value) {
    return value == null || value === "" || value == undefined || value.trim().length == 0;
}



//#region Jquery Validation


$.validator.addMethod("greaterThan",
    function (value, element, params) {
        //"value" & "params" must be "dd-mm-yyyy" format
        var fromdate = $(params).val().split("-");
        value = value.split("-");
        fromdate = fromdate[2] + "-" + fromdate[1] + "-" + fromdate[0];
        value = value[2] + "-" + value[1] + "-" + value[0];
        //var fromdate = $(params).val();
        if (!/Invalid|NaN/.test(value)) {
            return (value) >= fromdate;
        }
        return isNaN(value) && isNaN(fromdate)
            || (Number(value) > Number(fromdate));
    }, 'Must be greater than From Date.');

$.validator.addMethod("ChooseOne", function (value, element, arg) {
    return (null != value ? true : false)
}, "Please Choose One");

$.validator.addMethod("checkSpace", function (value, element, arg) {
    return (value.trim().length > 0 ? true : false)
}, "Please fill text");

$.validator.addMethod("isBackDate", function (value, element, arg) {
    const date = new Date();
    const year = date.getFullYear();
    const month = String(date.getMonth() + 1).padStart(2, '0');
    const day = String(date.getDate()).padStart(2, '0');
    const _today = year + month + day;
    const _date = value.split("-"); //Date.parse(value);
    const _value = _date[2] + _date[1] + _date[0];
    if (_value >= _today) {
        return true;
    } else {
        return false;
    }
}, "Can't fill back date!");

$.validator.addMethod('CheckAmount', function (value, element, param) {
    return GreaterThanZero(value);
}, 'Amount must be grater than 0 !');

$.validator.addMethod('CheckGraterThanZero', function (value, element, param) {
    return GreaterThanZero(value);
}, 'Value must be grater than 0 !');

$.validator.addMethod("isGreaterThan",
    function (value, element, params) {
        var toData = $(params).val();
        if (isNullOrEmpty(toData) || isNullOrEmpty(value)) {
            return true;
        }
        return value.toUpperCase() <= toData.toUpperCase();
    }, 'From data must be less than to data.');
$.validator.addMethod("customEmail", function (value, element) {
    var emailPattern = /^[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}$/i;
    return this.optional(element) || emailPattern.test(value);
}, "Enter a valid email address.");
function clearValidationMessage(p) {
    p.resetForm();
    $(".is-invalid").removeClass("is-invalid");
}

//#endregion


function formatDate(value) {
    let date = new Date(value);
    const day = date.toLocaleString('default', { day: '2-digit' });
    const month = date.toLocaleString('default', { month: '2-digit' });
    const year = date.toLocaleString('default', { year: 'numeric' });
    return day + '-' + month + '-' + year;
}

function checklength(txt, val) {
    var count = txt.trim().length;
    if (count > val) {
        return true;
    } else {
        return false;
    }
}


$.validator.addMethod("isGreaterThanToday", function (value, element, arg) {
    if ($('#ddFilter').val() === null) return true;
    var fromDate = $('#txtFromDate').data('datetimepicker').date();
    if (fromDate === null) return false;

    return fromDate.isSameOrBefore(moment(), 'day');
}, "Date can't be greater than today");

$.validator.addMethod("isGreaterThanToDate", function (value, element, arg) {
    if ($('#ddFilter').val() === null) return true;

    if ($('#txtToDate').data('datetimepicker').date() === null) return true;
    var fromDate = $('#txtFromDate').data('datetimepicker').date();
    var toDate = $('#txtToDate').data('datetimepicker').date();
    if (fromDate === null || toDate === null) return false;

    return fromDate.isSameOrBefore(toDate, 'day');
}, "Date can't be greater than the 'To' date");
$.validator.addMethod("CheckRequiredFilter", function (value, element, arg) {
    if ($('#ddFilter').val() === null) return true;
    return value !== '';
}, "Please choose a date!");

$.validator.addMethod("CheckRequiredFilter1", function (value, element, arg) {
    if ($('#ddFilter1').val() === null) return true;
    return value !== '';
}, "Please enter filter text!");
